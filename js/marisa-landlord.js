...
var SYMBOLS = {
  ...
  /* ── 优秀 r1:小引擎/辅助 ── */
  cirno:    { name: '琪露诺', e: '⑨', img: 'images/landlord/cirno.png', r: 1, v: 2, tags: ['角色'], d: '冻青蛙:消灭相邻青蛙,每个 +6。',
    fx: function(B, pos, X, ev) { var n = destroyAdj(B, pos, X, ev, { id: 'frog' }); return { add: n * (6 + X.eatBonus), msg: n ? '冻青蛙 ×' + n : null }; } },
  rumia:    { name: '露米娅', e: '🌑', img: 'images/landlord/rumia.png', r: 1, v: 2, tags: ['角色'], d: '消灭相邻动物,每个 +4。',
    fx: function(B, pos, X, ev) { var n = destroyAdj(B, pos, X, ev, { tag: '动物' }); return { add: n * (4 + X.eatBonus), msg: n ? '吃掉了 ×' + n : null }; } },
  ...
  rin:      { name: '阿燐',   e: '🐈‍⬛', img: 'images/landlord/rin.png', r: 1, v: 2, tags: ['角色'], d: '尸体搬运:消灭相邻妖怪,每个 +3。',
    fx: function(B, pos, X, ev) { var n = destroyAdj(B, pos, X, ev, { tag: '妖怪' }); return { add: n * (3 + X.eatBonus), msg: n ? '搬运尸体 ×' + n : null }; } },
  ...
  yuyuko:   { name: '幽幽子', e: '👘', img: 'images/landlord/yuyuko.png', r: 2, v: 3, tags: ['角色'], d: '吃掉相邻食材(+3)/料理(+5),每吃 1 个永久 +1 基础值。',
    fx: function(B, pos, X, ev) {
      ...
      for (i = 0; i < a.length; i++) {
        ...
        if (destroyCell(B, a[i], X, ev)) { gain += per + X.eatBonus; eaten++; }
      }
      if (eaten) { X.growYuyuko(eaten); ev.push({ pos: pos, kind: 'grow', msg: '幽幽子永久 +' + eaten }); }
  ...
  suika:    { name: '萃香',   e: '🍉', img: 'images/landlord/suika.png', r: 2, v: 3, tags: ['角色'], d: '消灭相邻清酒,每个 +6;场上有勇仪时再 +2。',
    fx: function(B, pos, X, ev) { var n = destroyAdj(B, pos, X, ev, { id: 'sake' }); return { add: n * (6 + X.eatBonus) + (B_hasId(B, 'yuugi') ? 2 : 0), msg: n ? '一饮而尽 ×' + n : null }; } },
  yuugi:    { name: '勇仪',   e: '👹', img: 'images/landlord/yuugi.png', r: 2, v: 3, tags: ['角色'], d: '酒豪:每个饮品 +2。',
    fx: function(B) { return { add: B_countTag(B, '饮品') * 2 }; } },
  eirin:    { name: '永琳',   e: '💊', img: 'images/landlord/eirin.png', r: 2, v: 3, tags: ['角色'], d: '调药:消灭相邻药草,每个 +6。',
    fx: function(B, pos, X, ev) { var n = destroyAdj(B, pos, X, ev, { id: 'herb' }); return { add: n * (6 + X.eatBonus), msg: n ? '调配合剂 ×' + n : null }; } },
  mokou:    { name: '妹红',   e: '🔥', img: 'images/landlord/mokou.png', r: 2, v: 3, tags: ['角色'], d: '不死鸟:被消灭时不消失,改为 +8(每转限 1 次)。',
    onDestroyed: function(B, pos, X, ev) {
      var c = B[pos];
      if (!c.revived) { c.revived = true; ev.push({ pos: pos, kind: 'grow', gain: 8, msg: '不死鸟复活 +8' }); return true; }
      return false;
    } },
  ...
  flandre:  { name: '芙兰朵露', e: '💥', img: 'images/landlord/flandre.png', r: 3, v: 4, tags: ['角色'], d: '495 年的波纹:每转随机消灭 1 个符号,+10。',
    fx: function(B, pos, X, ev, rnd) {
      ...
      if (destroyCell(B, t, X, ev)) return { add: 10 + X.eatBonus, msg: '破坏了 ' + SYMBOLS[B[t].id].name };
      return { add: 0 };
    } },
  ...
  utsuho:   { name: '阿空',   e: '☢️', img: 'images/landlord/utsuho.png', r: 3, v: 4, tags: ['角色'], d: '核爆:消灭周围 8 格,每个 +5,之后从收藏中消失。',
    fx: function(B, pos, X, ev) {
      var a = adjPos(pos), n = 0, i;
      for (i = 0; i < a.length; i++) if (destroyCell(B, a[i], X, ev)) n++;
      X.removeU(B[pos].u);
      ev.push({ pos: pos, kind: 'grow', msg: '☢️ 核爆!阿空力量耗尽' });
      return { add: n * (5 + X.eatBonus), msg: '核爆 ×' + n };
    } },
...
// 双向联动表:needs=我需要谁, givers=谁需要我
function linksFor(id) {
  var needs = [], seen = {}, givers = [], i, j, tid;
  ...
  return { needs: needs, givers: givers };
}

/* ═══════════ 4. 道具数据(22 种) ═══════════ */
var ITEMS = {
  orb:        { name: '阴阳玉',       e: '🔮', r: 0, d: '每次转动 +2 🪙', spinFlat: 2 },
  basket:     { name: '蘑菇篮',       e: '🧺', r: 0, d: '蘑菇 +1', addBase: { mushroom: 1 } },
  teaset:     { name: '红茶套装',     e: '🫖', r: 0, d: '所有饮品 +1', addTag: { '饮品': 1 } },
  toolbox:    { name: '河童工具箱',   e: '🧰', r: 0, d: '所有道具 +1', addTag: { '道具': 1 } },
  ...
  yakitori:   { name: '夜雀烤炉',     e: '🍢', r: 1, d: '所有食材 +1', addTag: { '食材': 1 } },
  menu:       { name: '白玉楼菜单',   e: '📜', r: 1, d: '料理 +2', addBase: { ryori: 2 } },
  kourindou:  { name: '香霖堂会员卡', e: '💳', r: 1, d: '跳过三选一时额外 +5 🪙', skipBonus: 5 },
  starvessel: { name: '星之器',       e: '🌟', r: 2, d: '所有自然 +1', addTag: { '自然': 1 } },
  hakkero:    { name: '迷你八卦炉',   e: '🔦', r: 2, d: '魔理沙 +3', addBase: { marisa: 3 } },
  saisenbox:  { name: '赛钱箱',       e: '📦', r: 2, d: '每次交租后返还 10% 租金', rentRefund: 0.1 },
  amulet:     { name: '金运御守',     e: '🧧', r: 2, d: '每次交租后 +15 🪙', rentCoins: 15 },
  gapumbrella:{ name: '隙间之伞',     e: '🌂', r: 2, d: '每次交租后 +1 重抽代币', rentReroll: 1 },
  moonrobe:   { name: '月之羽衣',     e: '🌙', r: 2, d: '传说符号 +2', addRarity: { 3: 2 } },
...
function dealBoard(S, rnd) {
  ...
  for (i = 0; i < CONFIG.CELLS; i++) {
    B.push(pool[i] ? { u: pool[i].u, id: pool[i].id, p: pool[i].p || 0 } : { id: 'empty', p: 0 });
  }
...
function makeCtx(S) {
  ...
  X.baseBonus = function(cell) {
    var b = 0, d = SYMBOLS[cell.id];
    for (var i = 0; i < S.items.length; i++) {
      var it = ITEMS[S.items[i]];
      if (it.addBase && it.addBase[cell.id]) b += it.addBase[cell.id];
      if (it.addTag) for (var t in it.addTag) if (hasTag(cell.id, t)) b += it.addTag[t];
      if (it.addRarity && it.addRarity[d.r]) b += it.addRarity[d.r];
    }
    return b;
  };
  X.dbl = function(id) {
  ...
  X.growYuyuko = function(n) {
    for (var i = 0; i < S.collection.length; i++) if (S.collection[i].id === 'yuyuko') S.collection[i].p += n;
  };
...
function resolveBoard(B, S, rnd) {
  ...
  for (pos = 0; pos < B.length; pos++) {
    ...
    if (!alive(cell)) continue;
    def = SYMBOLS[cell.id];
    var base = def.v + (cell.p || 0) + X.baseBonus(cell);
...
function genOffer(S, rnd) {
  rnd = rnd || Math.random;
  var w = CONFIG.RARE_W[S.day >= 6 ? 1 : 0];
  var out = [], guard = 0;
...
function showLinkPanel(id, instP) {
  ...
  if (!lk.needs.length && !lk.givers.length) body += '<div class="lp-sec" style="color:#8a7e9a;">暂无联动,朴实无华。</div>';
